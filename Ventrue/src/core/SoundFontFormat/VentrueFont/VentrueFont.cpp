...
namespace ventrue
{
    ...
    /// 解析乐器配置文件到ventrue 
    /// </summary>
    void VentrueFont::Parse(string filePath)
    {
        path = filePath;
    ...
    /// 解析单位类型
    /// </summary>
    void VentrueFont::ParseUnitType()
    {

        XMLElement* xmlVentrues = xmlDoc->FirstChildElement("Ventrue"); //取得节点名为Sample的XmlNode集合
        if (xmlVentrues == nullptr)
            return;

        const XMLAttribute* attr = xmlVentrues->FindAttribute("unitType");
        if (attr == nullptr)
            return;
        ...
        {
            unitType = VentrueXmlUnitType::UnitType_SoundFont;
        }
        else if (unitTypeStr.compare("Polyphone") == 0)
        {
            unitType = VentrueXmlUnitType::UnitType_Polyphone;
        }
        else if (unitTypeStr.compare("Ventrue") == 0)
        {
            unitType = VentrueXmlUnitType::UnitType_Ventrue;
        }
    }
    ...
    /// 解析样本列表    
    /// </summary>      
    void VentrueFont::ParseSampleList()
    {

        XMLElement* xmlSampleList = xmlDoc->FirstChildElement("Ventrue")->FirstChildElement("SampleList"); //取得节点SampleList
        ...
        for (; xmlSample; xmlSample = xmlSample->NextSiblingElement("Sample"))
        {
            ...
            Sample* sample = ventrue->AddSample(name, pcm, wavReader.GetDataSize());
    ...
    /// 解析乐器列表
    /// </summary>
    void VentrueFont::ParseInstrumentList()
    {
        SampleList& sampleList = *(ventrue->GetSampleList());
        XMLElement* xmlInstrumentList = xmlDoc->FirstChildElement("Ventrue")->FirstChildElement("InstrumentList"); //取得InstrumentList
        ...
        for (; xmlInstrument; xmlInstrument = xmlInstrument->NextSiblingElement())
        {
            ...
            Instrument* inst = ventrue->AddInstrument(name);
            ...
            for (; childRegionElem; childRegionElem = childRegionElem->NextSiblingElement("Region"))
            {
                ...
                else
                {
                    for (int i = 0; i < sampleList.size(); i++)
                    {
                        ...
                        {
                            region = ventrue->SampleBindToInstrument(sampleList[i], inst);
                            break;
                        }
                    }
                }
    ...
    /// 解析预设列表
    /// </summary>
    void VentrueFont::ParsePresetList()
    {
        InstrumentList& instList = *(ventrue->GetInstrumentList());

        XMLElement* xmlPresetList = xmlDoc->FirstChildElement("Ventrue")->FirstChildElement("PresetList"); //取得PresetList
        ...
        for (; xmlPreset; xmlPreset = xmlPreset->NextSiblingElement())
        {
            ...
            Preset* preset = ventrue->AddPreset(name, bankMSB, bankLSB, instNum);
            ...
            for (; childRegionElem; childRegionElem = childRegionElem->NextSiblingElement("Region"))
            {
                ...
                else
                {
                    for (int i = 0; i < instList.size(); i++)
                    {
                        ...
                        {
                            region = ventrue->InstrumentBindToPreset(instList[i], preset);
                            break;
                        }
                    }
                }
    ...
    bool VentrueFont::IsEqual(const char* s1, const char* s2)
    {
        return (strcmp(s1, s2) == 0);
    }

    void VentrueFont::SetGenListByXml(GeneratorList& genList, tinyxml2::XMLElement* elem, RegionType regionType)
    {
        string innerText;
        for (; elem; elem = elem->NextSiblingElement())
        {
            ...
            else
            {
                ...
                else if (IsEqual(name, "ModEnvToPitch")) { genList.SetAmount(GeneratorType::ModEnvToPitch, val); }
                else if (IsEqual(name, "InitialFilterFc"))
                {
                    switch (unitType)
                    {
                    case VentrueXmlUnitType::UnitType_SoundFont:
                ...
                else if (IsEqual(name, "ModLfoToVolume"))
                {
                    ...
                    case VentrueXmlUnitType::UnitType_Polyphone:
                        if (val > 96) val = 96;
                ...
                {
                    if (unitType == VentrueXmlUnitType::UnitType_SoundFont)
                    {
                        val = UnitTransform::TimecentsToSecsf(val);
                    }
                ...
                else if (IsEqual(name, "SustainModEnv"))
                {
                    ...
                    case VentrueXmlUnitType::UnitType_Polyphone:
                    case VentrueXmlUnitType::UnitType_Ventrue:
                        val /= 100;
                        break;
                    }
