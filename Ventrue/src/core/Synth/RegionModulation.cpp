...
namespace ventrue
{
    ...
    void RegionModulation::CombInstAndPresetGenList(bool isTestModFlag)
    {
        ...
        {
            if (isTestModFlag &&
                isModedInstGenTypes[i] == false &&
                isModedPresetGenTypes[i] == false)
                continue;
            ...
            {
                modifyedGenTypes[idx++] = (GeneratorType)i;
            }
        }

        modifyedGenTypes[idx] = GeneratorType::None;
    }
    ...
    {
        int i = 0;
        modifyedGenTypes[i++] = GeneratorType::SampleModes;
        modifyedGenTypes[i++] = GeneratorType::ScaleTuning;
        modifyedGenTypes[i++] = GeneratorType::InitialAttenuation;
        modifyedGenTypes[i++] = GeneratorType::Pan;
        modifyedGenTypes[i++] = GeneratorType::StartAddrsOffset;
        modifyedGenTypes[i++] = GeneratorType::EndAddrsOffset;
        modifyedGenTypes[i++] = GeneratorType::StartloopAddrsOffset;
        modifyedGenTypes[i++] = GeneratorType::EndloopAddrsOffset;
        modifyedGenTypes[i++] = GeneratorType::SustainPedalOnOff;

        if (!modifyedGenList->IsEmpty(GeneratorType::VibLfoToPitch))
        {
            modifyedGenTypes[i++] = GeneratorType::VibLfoToPitch;
            modifyedGenTypes[i++] = GeneratorType::DelayVibLFO;
            modifyedGenTypes[i++] = GeneratorType::FreqVibLFO;
        }

        if (!modifyedGenList->IsEmpty(GeneratorType::ModLfoToFilterFc))
            modifyedGenTypes[i++] = GeneratorType::ModLfoToFilterFc;

        if (!modifyedGenList->IsEmpty(GeneratorType::ModLfoToPitch))
            modifyedGenTypes[i++] = GeneratorType::ModLfoToPitch;

        if (!modifyedGenList->IsEmpty(GeneratorType::ModLfoToVolume))
            modifyedGenTypes[i++] = GeneratorType::ModLfoToVolume;


        modifyedGenTypes[i++] = GeneratorType::DelayModLFO;
        modifyedGenTypes[i++] = GeneratorType::FreqModLFO;


        bool isActiveModEnv = false;
        if (!modifyedGenList->IsEmpty(GeneratorType::ModEnvToPitch)) {
            modifyedGenTypes[i++] = GeneratorType::ModEnvToPitch;
            isActiveModEnv = true;
        }

        if (!modifyedGenList->IsEmpty(GeneratorType::ModEnvToFilterFc)) {
            modifyedGenTypes[i++] = GeneratorType::ModEnvToFilterFc;
            isActiveModEnv = true;
        }

        modifyedGenTypes[i++] = GeneratorType::AttackVolEnv;

        if (isActiveModEnv) {
            modifyedGenTypes[i++] = GeneratorType::AttackModEnv;
        }

        if (!modifyedGenList->IsEmpty(GeneratorType::InitialFilterFc) ||
            !modifyedGenList->IsEmpty(GeneratorType::InitialFilterQ))
        {
            modifyedGenTypes[i++] = GeneratorType::InitialFilterFc;
        }


        modifyedGenTypes[i] = GeneratorType::None;
    }
    ...
    void RegionModulation::ModGenList()
    {
        ...
        //insideCtrlModulatorList始终保持最高优先级处理，insideCtrlModulatorList中的复合方式都是replace
        SetGenListMods(insideCtrlModulatorList->GetModulators(), instCombGenList, instRegionModGenList, isModedInstGenTypes);
        SetGenListMods(instRegion->GetModulators(), instCombGenList, instRegionModGenList, isModedInstGenTypes);  
        ExecuteGenListMods(insideCtrlModulatorList->GetModulators(), instRegionModGenList, isModedInstGenTypes);
        ...
        ModulatorVec* mods, GeneratorList* modGenlist, bool* isAlreadyModedGenTypes)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            //获取调制值，执行调制
            modValue = mod.Output();
            isAlreadyModedGenTypes[(int)targetGenType] = Process(targetGenType, modValue, mod.GetOutModulationType());
        }
    }
    ...
    bool RegionModulation::ModulationAttenuation(float modValue, ModulationType modType)
    {
        ...
        return ModulationGenType(GeneratorType::InitialAttenuation, -modValue * 0.04f, modType);
    }
