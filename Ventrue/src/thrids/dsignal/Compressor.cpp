...
namespace dsignal
{
	...
	{
		attackCoffe = exp(-logf(9) / (attackSecLen * sampleFreq));
		releaseCoffe = exp(-logf(9) / (releaseSecLen * sampleFreq));

		if (isAutoMakeupGain)
		{
			makeupGain = -threshold + threshold / radio;
		}
	}
	...
	* 化解得到:
	* x = (x0-2x1+x2)*t^2 + 2(x1-x0)*t + x0
	* y = (y0-2y1+y2)*t^2 + 2(y1-y0)*t + y0
	* 代入已知点数值
	* 可知: x0-2x1+x2 = T-W/2 -2T + T+W/2 = 0
	*       x1-x0 = T - (T-W/2) = W/2
	*       x2 = T+W/2
	*
	*       y0-2y1+y2 = T-W/2 - 2T+T+W/2/R = W/2(R - 1)
	*       y1-y0 = T - (T-W/2) = W/2
	*       y2 = T+W/2/R
	*由上代入到4,5式，得到
	*          x = Wt + T+W/2                      (6)
	*          y = W/2(R-1)*t^2 + Wt + T+W/2/R     (7)
	*  6式带入到7式
	*  即可求出2式
	*
	* 平滑处理公式:
	...
	*          releaseCoffe = exp(-logf(9) / (releaseSecLen * sampleFreq));
	*/
	float Compressor::Tick(float value)
	{
		float x = abs(value);
		float xdB;
		if (x == 0)xdB = -140;
		else xdB = 20 * log10(abs(value));

		float xsc = xdB;

		if (kneeWidth != 0 &&
			xdB > threshold - kneeWidth / 2 &&
			xdB < threshold + kneeWidth / 2)
		{
			xsc = xdB + (1 / radio - 1) * FastPow2(xdB - threshold + kneeWidth / 2) / (2 * kneeWidth);
		}
		else if (xdB > threshold + kneeWidth / 2)
		{
			xsc = threshold + (xdB - threshold) / radio;
		}
		...
		if (gc > gs)
		{
			gs = attackCoffe * gs + (1 - attackCoffe) * gc;
		}
		else
		{
			gs = releaseCoffe * gs + (1 - releaseCoffe) * gc;
		}


		float glin = pow(10.0, (makeupGain + gs) / 20);
		return value * glin;
