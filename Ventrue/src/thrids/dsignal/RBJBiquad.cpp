...
namespace dsignal
{
	...
	void RBJBiquad::CalculateCoefficients()
	{
		double A = 0;
	...
	//直接设置分子分母系数
	void RBJBiquad::SetNumAndDenCoefficient(
	...
	void RBJBiquad::Filtering(float* inputs, uint32_t size)
	{
		if (!IsEnable())
			return;
	...
	double RBJBiquad::Filtering(double input)
	{
		if (!IsEnable())
			return input;
