...
Delay :: Delay( unsigned long delay, unsigned long maxDelay )
{
  ...
  if ( ( (size_t)maxDelay + 1 ) > inputs_.size() )
    inputs_.resize((size_t)maxDelay + 1, 1, 0.0 );
...
void Delay :: setDelay( unsigned long delay )
{
  ...
  else outPoint_ = (unsigned long) (inputs_.size() + inPoint_ - delay);
  delay_ = delay;
}
...
{
  size_t tap = inPoint_ - tapDelay - 1;
