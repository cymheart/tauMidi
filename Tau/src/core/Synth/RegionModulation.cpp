...
namespace tau
{
	...
	void RegionModulation::InitNeedModifyGenParamTypes()
	{
		...
		modifyedGenTypes->insert((int)GeneratorType::ChorusEffectsSend);
		...
		//modifyedGenTypes->insert((int)GeneratorType::Pressure);
	...
	void RegionModulation::ModGenList()
	{
		...
		if (useCommonModulator)
		{
			//首先使用区域调制器，来调制区域的instCombGenList
...
/// <param name="mods">用于调制输入生成器的调制器</param>
	...
	bool RegionModulation::Process(GeneratorType targetGenType, float modValue, ModulationType modType)
	{
		...
		{
		case GeneratorType::Pressure: ret = ModulationPressure(modValue, modType); break;
		...
		case GeneratorType::ChorusEffectsSend: ret = ModulationChorusDepth(modValue, modType); break;
	...
	// 调制按键压力
	bool RegionModulation::ModulationPressure(float modValue, ModulationType modType)
	{
		if (modType == ModulationType::Unknown)
			modType = ModulationType::Replace;

		return ModulationGenType(GeneratorType::Pressure, modValue, modType);
	}
	...
	// 调制器调制和声深度
	bool RegionModulation::ModulationChorusDepth(float modValue, ModulationType modType)
	{
		if (modType == ModulationType::Unknown)
			modType = ModulationType::Add;

		modValue *= 0.1f;
		return ModulationGenType(GeneratorType::ChorusEffectsSend, modValue, modType);
	}
