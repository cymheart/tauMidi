...
namespace tau
{
	...
	void ModulatorList::CreateInsideVolumeModulator()
	{
		...
		mod->SetOutModulationType(ModulationType::Add);
		mod->SetSourceTransform(0, VolGainTans, RangeFloat(0, 1), RangeFloat(0, 1));
		mod->SetAmount(1);
	...
	//生成内部PloyPressure调制器
	void ModulatorList::CreateInsidePloyPressureModulator()
	{
		Modulator* mod = new Modulator();
		mod->SetType(ModulatorType::Inside);
	...
	{
		return UnitTransform::GainToDecibels(gain) / 0.04f;
	}
