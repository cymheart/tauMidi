...
public class MidiInputPortSelector extends MidiPortSelector {
     ...
     * @param midiManager
     */
    public MidiInputPortSelector(MidiManager midiManager) {
        super(midiManager, MidiDeviceInfo.PortInfo.TYPE_INPUT);
    }
        ...
        if (info != null) {
            mMidiManager.openDevice(info, device -> {
                if (device == null) {
                    ...
                    mOpenDevice = device;
                    mInputPort = mOpenDevice.openInputPort(wrapper.getPortIndex());
                    if (mInputPort == null) {
