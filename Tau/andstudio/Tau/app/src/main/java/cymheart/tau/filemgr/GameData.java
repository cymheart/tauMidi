...
public class GameData {
    ...
    protected JSONObject[] jsonGameModes = new JSONObject[3];
    ...
    /**游戏模式信息*/
    public List<GameModeInfo> gameModeInfos = new ArrayList<>();
    ...
    protected void CreateJson() throws JSONException
    {
        ...
        jsonVerData.put("cover", "");

        //
        for(int i=0; i<3; i++) {
            jsonGameModes[i] = new JSONObject();
            jsonGameModes[i].put("left_hand", -1);
            jsonGameModes[i].put("right_hand", -1);
            jsonGameModes[i].put("double_hand", -1);
            jsonVerData.put("game_mode_" + i, jsonGameModes[i]);
        }
    ...
    {
        String jsonStr = FileUtils.getInstance().ReadFileToString(gameFilePath);
        if(jsonStr == null){
            CreateJson();
            return;
        }
        ...
        //
        coverImagePath = jsonVerData.getString("cover");
        //
        jsonGameModes[0] = jsonVerData.getJSONObject("game_mode_0");
        jsonGameModes[1] = jsonVerData.getJSONObject("game_mode_1");
        jsonGameModes[2] = jsonVerData.getJSONObject("game_mode_2");
        //
        ...
        //game mode
        for(int i=0; i<3; i++) {
            if (jsonGameModes[i] != null) {
                GameModeInfo gameModeInfo = new GameModeInfo();
                gameModeInfos.add(gameModeInfo);
                gameModeInfo.mode = 0;
                gameModeInfo.score[0] = (float)jsonGameModes[i].getDouble("left_hand");
                gameModeInfo.score[1] = (float)jsonGameModes[i].getDouble("right_hand");
                gameModeInfo.score[2] = (float)jsonGameModes[i].getDouble("double_hand");
            }
        }
